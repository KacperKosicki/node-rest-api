...
const OrderTicketForm = () => {
  ...
  const submitForm = async (e) => {
    ...
    if (order.client && order.email && order.day && order.seat) {
      ...
      // Uruchom ponownie ładowanie miejsc po zarezerwowaniu biletu
  ...
  return (
    <Form className="order-ticket-form" onSubmit={submitForm}>
      <Row>
        <Col xs="12" md="6">
          {isError && <Alert color="warning">There are some errors in your form. Have you filled all the fields? Maybe you forgot to choose your seat?</Alert>}
          {requests['ADD_SEAT'] && requests['ADD_SEAT'].error && !isError && <Alert color="danger">{requests['ADD_SEAT'].error}</Alert>}
          {requests['ADD_SEAT'] && requests['ADD_SEAT'].success && !isError && <Alert color="success">You've booked your ticket! Check your email to make a payment.</Alert>}
          ...
          <FormGroup>
            <Label for="clientDay">Select which day of festivals you are interested in:</Label>
            <Input type="select" value={order.day} name="day" onChange={updateNumberField} id="exampleSelect">
              <option>1</option>
              <option>2</option>
              <option>3</option>
            </Input>
            <small id="dayHelp" className="form-text text-muted">Every day of the festival uses an individual ticket. You can book only one ticket at a time.</small>
          </FormGroup>
          <FormGroup check>
            <Label check>
